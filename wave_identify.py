...
def taper_func(nc,ntime=45,kind='start'):
    '''
    function to apply taper to start ntime time points (and end ntime time points) of data
    nc : xarray DataArray
...
def filt_project_func(u,gh,v,
    ...
    pmin, pmax, kmin and kmax are set to defaults. zonal wavenumber k=2-40 and period 2-30 days
    ...
    taper_days:        Number of days to taper from start (and end) of data
    taper_kind:        'start' or 'both' to taper start of data (real time) or both ends of data (historical)
    ...
    # conversion to u,v,z for each wave type
    uf_wave=[]
    zf_wave=[]
    vf_wave=[]
    ...
    #Kelvin wave q as mode=0
    qf_mode = qf_mode.copy(data =np.concatenate( [[qf_Kel.values],qf_mode.values[1:]]))

    #can optionally return q,r,v coefficients only, this saves space as no latitude dimension - you can convert to u,v,z separately when the data needs to be used, as above.
    #return qf_mode, rf_mode, vf_mode
